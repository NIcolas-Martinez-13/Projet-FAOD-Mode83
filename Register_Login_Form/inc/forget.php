...
<?php if(!empty($_POST) && !empty($_POST['email'])) {
    ...
    if($user) {
        ...
        $pdo -> prepare('UPDATE utilisateurconnecte SET reset_token = ?, reset_at = NOW() WHERE idUser = ?') -> execute([$reset_token, $user->id]);
        $_SESSION['flash']['success'] ='Les instructions du rappel de mot de passe vous ont été envoyées par email';
        mail($_POST['email'], "Réinitialisation de votre mot de passe", "Affin de réinitialiser votre mot de passe, merci de cliquer sur ce lien\n \n http://localhost:8888/reset.php?id={$user->id}&token=$reset_token");
