...
if(isset($_GET['id']) && isset($_GET['token'])) {
    ...
    if($user) {
        if(!empty($_POST)) {
            if(!empty($_POST['password']) && $_POST['password'] == $_POST['confirm_password']) {
                ...
                $pdo->prepare('UPDATE utilisateurconnecte SET password = ?, reset_at = NULL, reset_token = NULL')->execute([$password]);
                session_start();
                $_SESSION['auth'] = $user;
    ...
    header('Location: login.php');
    $_SESSION['flash']['error'] = "Une erreur est arrivé, veuillez rééssayer";
    exit();
}
?> 
